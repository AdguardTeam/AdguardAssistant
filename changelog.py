...
def get_gh_issue_summary(branch_name_parts, repo, token):
    ...
    issue_url = BASE_URL + repo + "/issues/" + branch_name_parts.group(2)
    req = urllib2.Request(
            url=issue_url,
            headers={
    ...
    patch_name = branch_name_parts.group(3)
...
if __name__ == '__main__':
    ...
    for line in merged_branches.splitlines():
        ...
        if branch_name_parts:
            if not unicode(branch_name_parts.group(2), 'utf-8').isnumeric():
                continue
